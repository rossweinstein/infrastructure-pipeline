...
node('linux') {
    ...
    stage ("TerminateInstance") {
        def output = sh returnStdout: true, script: 'aws ec2 describe-instances --region us-east-1 --filters "Name=key-name,Values=seis665-classroom-key,Name=instance-state-name,Values=pending" | jq .Reservations[].Instances[].InstanceId'
