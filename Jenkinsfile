...
node('linux') {
    ...
    stage ("CreateInstance") {
        ...
        sh "aws ec2 describe-instances --region us-east-1 --image-id ami-467ca739 --instance-type t2.micro --key-name seis665-classroom-key --security-group-ids sg-c6d452b1 --subnet-id subnet-f6f4b2c9 | jq .Reservations[].Instances[].InstanceId"
    }

    stage ("TerminateInstance") {
        def output = sh returnStdout: true, script: 'aws ec2 describe-instances --region us-east-1 | jq .InstanceId'
